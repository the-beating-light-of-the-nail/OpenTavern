...
<template>
  <AppModal modal-id="groupMentionModal" name="groupMention" max-width="max-w-md" closable>
    <h2 class="ui-modal-title mb-3" style="color:var(--color-text)">{{ t('group_mention_title') }}</h2>
    <div id="groupMentionList" class="max-h-[55vh] overflow-y-auto pr-1 space-y-1.5">
      <div
        v-for="(m, i) in members"
        :key="i"
        class="ui-panel ui-panel-flat rounded-xl px-3 py-2 text-sm transition-all flex items-center gap-2"
        style="color:var(--color-text);border-color:var(--color-primary)"
        :class="m.muted ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'"
        @click="choose(m)"
      >
        <div class="w-8 h-8 rounded-lg ot-avatar-fill flex items-center justify-center text-[var(--color-on-primary)] text-xs font-bold flex-shrink-0">{{ (m.character?.name || '?').charAt(0) }}</div>
