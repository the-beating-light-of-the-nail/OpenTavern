...
<template>
  <AppModal modal-id="quickNewChatModal" name="quickNewChat" max-width="max-w-sm" closable>
    <h2 class="text-lg font-bold text-zinc-100 mb-4">{{ t('quick_new_chat_title') }}</h2>
    <div class="space-y-2.5">
      ...
      <button id="quickNewGroupChatBtn" class="w-full btn-ghost border border-white/10" @click="onNewGroup">{{ t('quick_new_group') }}</button>
    </div>
  </AppModal>
</template>
