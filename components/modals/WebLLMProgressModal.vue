...
<template>
  <AppModal modal-id="webllmProgressModal" name="webllmProgress" max-width="max-w-md" :backdrop-close="false" :closable="false">
    <div class="flex items-center justify-between mb-3">
      <h2 class="text-lg font-bold" style="color:var(--color-text)">{{ t('webllm_progress_title') }}</h2>
    </div>
    ...
    <div class="mt-4 flex gap-2">
      <button id="webllmDetectDeviceBtn" class="flex-1 btn-ghost border" style="color:var(--color-primary);border-color:var(--color-border)" @click="onDetect">{{ t('webllm_detect_device') }}</button>
